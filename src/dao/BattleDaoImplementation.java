	...
	public void createBattle(Battle battle) throws Exception {		
		try {
			...
			while(rs.next()) {
				...
				battle.setBattleDate(rs.getString("battle_date"));
			}
	...
	public List<Battle> listBattles(String uidUser) throws Exception {
		...
		try {
			...
			String qry = " SELECT id_battle, id_winner, id_loser, id_chosen, skill, " +
						 " google_uid_user, DATE_FORMAT(battle_date, '%d/%m/%Y - %H:%i') as battle_date FROM battles " +
						 " where google_uid_user = ? ORDER BY battle_date DESC";
